...
def init_db():
    conn=get_db(); conn.execute("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, name TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
...
def criar_conta_page():
    return send_from_directory(".","criar-conta.html")
...
def login():
    ...
    if user and check_password_hash(user["password_hash"],password):
        session.clear(); session["authenticated"]=True; session["username"]=user["username"]; session["name"]=user["name"]
...
def criar_usuario():
    ...
    conn=get_db()
    try:
        conn.execute("INSERT INTO users (username,password_hash,name) VALUES (?,?,?)",(username,generate_password_hash(password),name)); conn.commit()
    except sqlite3.IntegrityError:
        conn.close(); return {"success":False,"message":"Esse usuário já existe."},409
    conn.close()
...
def excluir_usuario(user_id):
    conn=get_db(); user=conn.execute("SELECT username FROM users WHERE id=?",(user_id,)).fetchone()
    if user and hmac.compare_digest(user["username"],session.get("username","")):
        conn.close(); return {"success":False,"message":"Você não pode excluir o usuário conectado."},400
    conn.execute("DELETE FROM users WHERE id=?",(user_id,)); conn.commit(); conn.close(); return redirect(url_for("usuarios"))

@app.get("/api/usuarios")
@login_required
def listar_usuarios():
    conn=get_db(); users=conn.execute("SELECT id,name,username,created_at FROM users ORDER BY id").fetchall(); conn.close(); return {"users":[dict(u) for u in users]}
